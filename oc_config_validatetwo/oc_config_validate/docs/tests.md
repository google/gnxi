## How to create oc_config_validatetwo tests

`oc_config_validatetwo` executes the test listed in a YAML file with the following structure:
...
- !TestCase
  ...
  name: [:string]
  # A valid class in the oc_config_validatetwo.testcases module
  class_name: [:string]
...
the `oc_config_validatetwo.testcases` module, that tests something (it can be as simple as testing a gNMI Get is successful).
...
> Some `oc_config_validatetwo` tests files for common network devices are already curated in the [tests/](../tests) folder.
