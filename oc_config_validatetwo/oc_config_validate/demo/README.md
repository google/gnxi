...
1. It runs `oc_config_validatetwo` with [sample test files](demo/*_tests.yaml).

1. It writes the results to [JSON file](demo/*_results.json).

## On Docker

Run the demo of `oc_config_validatetwo` in a Docker container:

```
docker pull joseignaciotamayo/oc_config_validatetwo:demo_latest

docker run -it joseignaciotamayo/oc_config_validatetwo:demo_latest
 ...
 * The script assumes `oc_config_validatetwo` and its dependencies are installed.
