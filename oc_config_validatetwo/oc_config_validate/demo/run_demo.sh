...
# start_oc_config_validatetwo <gnmi_port>
start_oc_config_validatetwo() {
    OPTS=""
    if [[ "$NO_TLS" -eq 1 ]]; then
        OPTS="--no_tls"
    fi
    ...
    for t in config telemetry; do
      echo
      echo "--- Run oc_config_validatetwo $OPTS for $t"
      echo
      PYTHONPATH="$PYTHONPATH:${BASEDIR}/.." python3 -m oc_config_validatetwo \
