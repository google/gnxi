...
```
python3 -m oc_config_validatetwo --version
python3 -m oc_config_validatetwo -models
```

## Update OC models

To get the latest OC Models from GitHub, running `oc_config_validatetwo/update_models.sh`.

This will take some time, be patient.

### Use specific OpenConfig models

If specific OpenConfig models need to be used, place them in a directory (here `$MODELS_DIRECTORY`) and have **pyang** create the bindings and store them in `oc_config_validatetwo/models/` (also write their revisions in `oc_config_validatetwo/models/versions`):

See [docs/oc_models.md](docs/oc_models.md) for details.

## Augmenting `oc_config_validatetwo`

There are 2 main ways to contribute to this tool:

### Write testcases

With a little of Python, you can extend the testcases that oc_config_validatetwo runs. Compare this to writing [unittest.TestCase](https://docs.python.org/3/library/unittest.html#basic-example) classes (see [docs/testcases.md](docs/testcases.md) for details).
...
For instance:
 ...
 > Remember that *oc_config_validatetwo* applies an initial JSON-formated configuration to the device, before running any test.
...
## Releasing new versions or `oc_config_validatetwo`

1. Update the OC models, by running `oc_config_validatetwo/update_models.sh`
1. Rebuild the gNMI generated code, with `oc_config_validatetwo/gnmi/make_gnmi_proto_py.sh`
1. Update the version of oc_config_validatetwo, by editing `__init__.py`
1. Run the demo to verify it all works, by running `demo/run_demo.sh`
1. Run `oc_config_validatetwo` with a real gNMI target to verify it all works

### Releasing the `oc_config_validatetwo` PyPi package
...
1. Verify that the package installs correctly, with `pip3 install dist/oc_config_validatetwo-${VERSION?}.tar.gz` and `pip3 show oc_config_validatetwo`.
1. Verify that the package runs correctly, with `python3 -I -m oc_config_validatetwo --version`.
1. Upload the latest packages to PyPi with `python3 -m twine upload --verbose dist/oc_config_validatetwo-${VERSION?}*`.

### Releasing the `oc_config_validatetwo` Docker images

The Docker images take the latest code from the `master` branch of the Git repository.

1. Create a Docker Image for the latest version of `oc_config_validatetwo`, tagged with the version number and `latest`.

   ```
   cd oc_config_validatetwo/docker
   sudo docker build --no-cache -t joseignaciotamayo/oc_config_validatetwo:<version> -t joseignaciotamayo/oc_config_validatetwo:latest .
   ```

1. Verify that the docker container runs with `sudo docker run joseignaciotamayo/oc_config_validatetwo:latest`.

1. Push the Docker Images for with both tags

   ```
   sudo docker push joseignaciotamayo/oc_config_validatetwo:<version>
   sudo docker push joseignaciotamayo/oc_config_validatetwo:latest
   ```
