...
from oc_config_validatetwo import models
from oc_config_validatetwo.gnmi import gnmi_pb2  # type: ignore
...
def ocContainerFromPath(model: str, xpath: str) -> PybindBase:
    ...
    This method look for the model class in the oc_config_validatetwo.models
      package

    Args:
        model: the OC model class name in the oc_config_validatetwo.models
...
def ocLeafFromPath(model: str, xpath: str) -> Any:
    ...
    This method look for the model type in the oc_config_validatetwo.models
      package
...
def _ocObjFromPath(model: str, xpath: str) -> Any:
    ...
    model_mod = importlib.import_module(
        "oc_config_validatetwo.models." + parts[0])
    model_cls = getattr(model_mod, parts[1])
    if not isclass(model_cls):
        raise ModelClassError(
            "%s is not a class in oc_config_validatetwo.models package" % model)
